...
template <typename T>
inline void convert_region_to_pts(T const * const roi, T * pts)
{
    T cx = roi[0];
    T cy = roi[1];
    T w = roi[2];
...
#if 1
    float rect_pts[8];
    convert_region_to_pts(roi, rect_pts);
    std::vector<cv::Point> points {
        {rect_pts[0], rect_pts[1]}, {rect_pts[2], rect_pts[3]}, {rect_pts[4], rect_pts[5]}, {rect_pts[6], rect_pts[7]}
    };
    cv::fillConvexPoly(out,               //Image to be drawn on
                   points,                 //C-Style array of points
                   cv::Scalar(1),  //Color , BGR form
...
int main(int argc, char const *argv[])
{
    ...
    {
        mask_iou = compute_rotate_mask_iou(gt_mask, proposal);
    }
