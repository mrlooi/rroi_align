...
{   
    ...
    int num_of_inter = 0;
    T int_pts[16];
    num_of_inter = inter_pts(rect_pts1, rect_pts2, int_pts);
    // printf("num_of_inter: %d\n", num_of_inter);

    reorder_pts(int_pts, num_of_inter);

    T interArea = area(int_pts, num_of_inter);
#else
...
template <typename T>
__DEVICE__ inline void reorder_pts(T * int_pts, int num_of_inter) {
...
#if 1
  T area_inter = inter(region1, region2);
#else
  T rect_pts1[8];
  T rect_pts2[8];
